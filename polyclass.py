...
class polyclass:
    ...
    def opemat(self,kope='',op1='',op2=''):
        '''
            opemat(kope,op1,op2)
                kope=string 's'- simplify, 'f' - factor  etc  
                op1= value to apply 1= left part, 2= right part of EQ
                op2= 1 = update
                
            example:
            Eq1.opemat('sf','1',1) simplify then factor only left side of Eq then update
        '''
        if op1=='1':
    ...
    def ope4( self,op1='',op2=0,kd='0',kope=''):
        '''
            ope4(op1,op2,kd,kope)
                op1=  Method to apply 'S' add,'M' mul ,'D' divide ,'P' pow 
                op2=  value to apply  
                kd = 1 = update
                kope=opemat opction
                
            example:
            Eq1.ope4('M',3*m,1,s) mul Eq by 3*m, simplify and update
        '''
        kres=self.Q
    ...
    def solve(self,op1,kd='',kpositive=False,korden='',kope=''):
        self.unifique_exp()
        kres=self.q1-self.q2
        if kd!='':
            kres=csolve(kres,op1,kd,kpositive=kpositive,korden=korden)
        else:
            kres=csolve(kres,op1,kpositive=kpositive,korden=korden)
        kres=opemat(kres,kope=kope)
        return(kres)
        
    def psolve(self,ksym):
        Eq1=self.q1-self.q2
        kres=ksolve(Eq1,ksym)
        return kres
...
#  ********  new simplify ****
    def psimplify(self,op1='',op2='',kope=''):
        kres1=self.q1
        kres1=operacion(kres1,op1=op1,op2=op2,kope=kope)
        kres2=self.q2
        kres2=operacion(kres2,op1=op1,op2=op2,kope=kope)
        self.q1=kres1
        self.q2=kres2
        self.updateQ()
        return(self.Q)
        
    def setValue(self,ksym,kexp,kope=''):
        ksym=unisymbols(ksym)
        kexp=unisymbols(kexp)
        kq1=unisymbols(self.q1)
        self.q1=ksubs(kq1,ksym,kexp,kope='')
        kq2=unisymbols(self.q2)
        self.q2=ksubs(kq2,ksym,kexp,kope='')
        self.updateQ()
        return(self.Q)
...
class Mpolyclass:
    ...
    def get_data(self,vec):
        if vec[1]==2:
            return self.q2[vec[0]]
        else:
            return self.q1[vec[0]]
    ...
    def del_eQ(self,op,kshow=False):
        mm=[]
        for i in range(self.qq):
            if i!=op:
                mm.append(polyclass(self.q1[i],self.q2[i]))
        #self=Mpolyclass(mm)
        newM=Mpolyclass(mm)
        self=newM
        # for i in mm:
    ...
    def clean_sys(self,kshow=False):
        i=0
        done=True
        qq=self.qq
        while done:
            try:
                ...
                if (self.q1[i]-self.q2[i])==0 :
                    self.del_eQ(i,kshow=kshow)
...
#********* algoritmo *******
    ...
    def change_var(self,vksym,vkval):
        for ksym,kval in zip(vksym,vkval):
             
            for i in range(self.qq):
                e1=self.Q[i] 
                e2=e1.subs(unisymbols(ksym),kval)
                self.q1[i]=e2.args[0]
                self.q2[i]=e2.args[1]
                self.up_class()
        return(self.showQ()) 
    ...
    def apli_simp_solu(self,op=0,kshow=True):
        ksym=unisymbols(self.q1[op])
        kval=unisymbols(self.q2[op])
        for i in range(self.qq):
            if i!=op:
                kres1=unisymbols(self.q1[i])
                kres2=unisymbols(self.q2[i])
                try:
    ...
    def subsolve2(self,ksym,kitem,kremp=True,kshow=True): # solve ksym in eqNum kitem and remplaze
        ee1=self.q1[kitem]
        ee2=self.q2[kitem]
        list1=fpoly(ee1,'free')
        list2=fpoly(ee2,'free')
        if ksym not in list1 and ksym in list2:
            self.swap(kitem)
    ...
    def try_solve(self,vksym='',vkitem='',kremp=True,tkshow=True):
        if vksym=='':
            vksym=self.get_freelist()
            vkitem=[x for x in range(len(vksym))]
        for ksym,kitem in zip(vksym,vkitem):
            self.subsolve2(ksym,kitem,kremp=kremp,kshow=False)
        if tkshow:
            self.showQ()
            
    def psolve(self,ksym,kitem,kremp=True,kshow=True):
        ee1=self.q1[kitem]
        ee2=self.q2[kitem]
        list1=fpoly(ee1,'free')
        list2=fpoly(ee2,'free')
    ...
    def Msolve(self,vecs,vecn,kremp=True,kshow=True):
        for ksym,kitem in zip(vecs,vecn):
            self.psolve(ksym,kitem,kremp=kremp,kshow=False)
        if kshow:
            self.showQ()    
