...
def operacion(ksym,op1='',op2='',kope=''):
        kres=ksym
        if op1 in 'SMDP' and op2!='':
...
def Mpsimplify(Vksym,op1='',op2='',kope=''):
    ktype=type(Vksym)
    if ktype==Add:
        kres=0
        for i in fpoly(Vksym,'list'):
            ki=psimplify(i,op1=op1,op2=op2,kope=kope)
            kres+=ki
        return kres
    else:
        return Vksym
    
def do_with_args(ksym,karg,knewval,kope=''):
    kres=unisymbols(ksym)
    oldv=kres.args[kargs]
    newv=knewval
    nkres=kres.subs(oldv,newv)
    return nkres
