...
class mparticle:
    def __init__(self,x1=x1,x2=x2,y1=y1,y2=y2,v1=v1,v2=v2,m=m,a=0,ang=0,g=g,v=v,w=w,ac=0,s='r',t=t,r=r,r1=r1,r2=r2,vx='',vy='',vxy=vxy,Ti=Ti,Io=Io,aw=aw,w1=w1,w2=w2 ,at=at,ax=0,ay=0,typeI='',mu=mu, Nm=Nm,Itype='p',xI=0,dire=0,direx=0,direy=direy,vv='',deltat=0.01,posx=0,posy=0,dt=dt,Ma=Ma):
    ...
    def x_res(self,kope='',keval=True,kunisym=True,**kwargs): # ********   Static Result sum F 
    ...
    def get_yvel(self,**kwargs):
        if len(kwargs)>0:
            vsym,vval=unpack(kwargs)
            if 't' in vsym:
                v1=self.vy
                gg=self.g
                kres=v1-gg*t/2
            ...
            return self.y_vel(t=t,kope='',keval=True)
    ...
    def xpos(self,*args,**kwargs):
        ...
        if self.v2!=v2:
            ...
            ac=self.ac           
        kres=float2int(self.x1+tt*self.v1*cos(self.a)+ac*tt*tt/2)
        return smartanswer(kres, *args,**kwargs)
    ...
    def y_max(self,**kwargs):
        return self.ymax(**kwargs)
    
    def ymax(self,**kwargs):
        g,a,v1,y1=self.g,self.a,self.v1,self.y1
        kres=rsimplify(y1+(v1**2)*(sin(a)**2)/(2*g))
        kres= real_subs(kres,**kwargs)
        return kres
    ...
    def tfly(self,*args,**kwargs):
        kname=firstdataname(*args)
        tt=t
        v1,g,a,y1,y2=self.v1,self.g,self.a,self.y1,self.y2
        kres=rsimplify((v1*sin(a) + sqrt(2*g*y1 - 2*g*y2 + v1**2*sin(a)**2))/g)
        kres=real_subs(kres,**kwargs) 
        return smartreturn(kres,name=kname,var=t)
