...
m,m0,m1,m2,m3,m4,M,g,x,x1,x2,y,y1,y2,X,Y,a,a1,a2,a3,v,v1,v2,M1,M2,M3, V ,V1 ,V2= symbols('m m0 m1 m2 m3 m4 M g x x1 x2 y y1 y2 X Y a a1 a2 a3 v v1 v2 M1 M2 M3 V V1 V2 ')
...
t,vx,vy,vxy,Po,Ti,I_n,In,a_w,Fc,a_t=symbols('t vx vy vxy Po Ti In In aw Fc  at ')
...
vv,Ma=symbols('vv  Ma') # velocidad al cuadrado
# diff variables
dt=symbols('d_t') 
...
q,q1,q2,d,K,vx,vy=symbols('q,q1 q2 d K vx vy')
...
class mparticle:
    ...
    def Fx(self,kname='Fx',kshow=True):
        ...
        return ee
    def xres(self,**kwargs):
        return self.x_res(**kwargs)
    def yres(self,**kwargs):
        return self.y_res(**kwargs)
    def xyres(self,**kwargs):
        return self.xy_res(**kwargs)        
    ...
    def x_res_relative(self,kang,kname='',kope='s',kshow=True):
        ...
        if kname!='':
            ...
            return kres
    def Frelative(self,kang,kname=''):
        return self.x_res_relative(kang,kname=kname)
    ...
    def energy(self,ktype='',**kwargs):
        return self.energia(ktype=ktype,**kwargs)
    ...
    def Etotal(self,op=''):
        ...
        if '1' in op or '2' in op:
            if '1' in op and not '2' in op:
                ...
                if 'p' in op:
                    kres+=self.energia('p2')-self.energia('p1')
                if 'k' in op:
                    kres+=self.energia('k2')-self.energia('k1')
                if 'r' in op:
                    kres+=self.energiaRot('ro2')-self.energiaRot('ro1')
