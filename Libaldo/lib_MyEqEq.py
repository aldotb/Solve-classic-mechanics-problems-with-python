...
class MyEqEq:
    def __init__(self, *args, var=x , show=True):
        ...
        self.bK1=[self.e1.ksym]
        self.bK2=[self.e2.ksym]
        if show:
            self.s()
    ...
    def undo(self,k=2):
        ...
        self.e2.ksym=p2
        ps=self.ssym
        self.bK1=self.bK1[0:-2]
        self.bK2=self.bK2[0:-2]
        display(Math(latex(p1)+' '+ps+' '+latex(p2)))
    ...
    def tsimplify(self,OPS='RL',show=True):
        if 'L' in OPS:
            self.e1.ksym=trigsimp(self.e1.ksym)
        if 'R' in OPS:
            self.e2.ksym=trigsimp(self.e2.ksym)    
        if show:
            self.s()
            
    def texpand(self,OPS='RL',show=True):
        if 'L' in OPS:
            self.e1.ksym=expand_trig(self.e1.ksym)
        if 'R' in OPS:
            self.e2.ksym=expand_trig(self.e2.ksym)    
        if show:
            self.s()
    ...
    def simpli(self):
        ...
        if Is_Pow(p1) and Is_Pow(p2):
            b1,e1=getbase(p1),getexpo(p1)
            b2,e2=getbase(p2),getexpo(p2)
            if b1==b2:
                p1=e1
                p2=e2
            elif e1==e2:
    ...
    def simplify(self,OPS='RL',show=True):
        ...
        if Is_Add(self.R) and Is_Add(self.L):
            vec1=list(self.e1.ksym.args)
            vec2=list(self.e2.ksym.args)
            for data in vec1:
                if data in vec2:
                    vec1.remove(data)
                    vec2.remove(data)
        ...
        if Is_Mul(self.R) and Is_Mul(self.L):
            vec1=list(self.e1.ksym.args)
            vec2=list(self.e2.ksym.args)
            for data in vec1:
                if data in vec2:
        ...
        if Is_Pow(self.R) and Is_Pow(self.L):
            b1,e1=getbase(self.R),getexpo(self.R)
            b2,e2=getbase(self.L),getexpo(self.L)
            if b1==b2:
                self.e1.ksym=e1
                self.e2.ksym=e2
            if e1==e2:
    ...
    def diff(self,var1,var2,show=True):
        dvar1=symbols('d'+str(var1))
        dvar2=symbols('d'+str(var2))
...
def MQ(expr1,expr2,*args, var=x , show=True): 
    if type(expr2)!=str:
        return MyEqEq(expr1,expr2,*args)
    else:
...
def superset(*args,**kwargs):
    ...
    # to change value into MyEqEq in group
    # superset(Q1,Q2,Q3...,x=1,y=3) , change variable value
    # superset(Q1,Q2,Q3....[x+y,z]) , change expresion value
    # superset(Q1,Q2,Q4...."function(args...,sjhow=False)", aplieed function
    ...
    for data in args:
        if type(data)==list:
            ...
            vec.append(data)
    for data in vec:        
        data.set(**kwargs,show=False)

    for data in igu:
        op1,op2=data
        for data2 in vec:
            try:
                data2.subs(unisymbols(op1),op2,show=False)
                data2.simplify(show=False)
            except:
                pass
    for ope in opera:
        for data in vec:
            if ope=="unisymbols":
                try:
                    data.unisymbols(show=False)
                except:
                    pass
            elif:
                ope=="simplify":
                try:
                    data.simplify(show=False)
                except:
                    pass    
            else:        
