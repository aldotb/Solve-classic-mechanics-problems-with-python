...
# 🔇 SILENCIAR WARNINGS DE NUMPY
warnings.filterwarnings("ignore", category=RuntimeWarning, module="numpy")
warnings.filterwarnings("ignore", category=RuntimeWarning, module="lambdifygenerated")
...
class MyPlotC:
    def __init__(self, *args, x_min=None, x_max=None, y_min=None, y_max=None, samples=1000, tam=1.0,hline='',vline=''):
        functions = []
        vecf = []
    ...
    def _safe_lambdify(self, f):
        ...
        def wrapper(x_array):
            try:
                # 🔇 SILENCIAR WARNINGS DENTRO DE LA FUNCIÓN
    ...
    def _plot(self):
        ...
        # 🔥 CORREGIR: usar self.hline y self.vline
        if self.hline != '':
            if type(self.hline) != tuple:
                self.ax.axhline(y=self.hline, color='r', linestyle='--')  # 🔥 También corregí x= por y=
            else:
                for data in self.hline:
                    self.ax.axhline(y=data, color='r', linestyle='--')    # 🔥 También corregí x= por y=
                        
        if self.vline != '':
            if type(self.vline) != tuple:
                self.ax.axvline(x=self.vline, color='r', linestyle='--')  # 🔥 También corregí y= por x=
            else:
                for data in self.vline:
                    self.ax.axvline(x=data, color='r', linestyle='--')    # 🔥 También corregí y= por x=
...
HH=''
VV=''    
def MyPlot(*args, xmin=None, xmax=None, ymin=None, ymax=None, samples=1000,tam=1,hline=HH,vline=VV):
    '''
    ...
    PP = MyPlotC(*vdata, 
                 ...
                 tam=tam,
                 hline=hline,
                 vline=vline)  # 🔥 Pasar factor de tamaño