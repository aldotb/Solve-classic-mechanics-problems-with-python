...
m,g,x1,x2,y1,y2,v1,v2,g, t, a,ac,v,r,vx,vy,vxy,w,Po,Ti=symbols('m g x1 x2 y1 y2 v1 v2 g t a  ac v r vx vy vxy w Po Ti')
 

class mparticle:
    def __init__(self,x1=x1,x2=x2,y1=y1,y2=y2,v1=v1,v2=v2,m=m,a=a,g=g,v=v,ac=ac,s='r',t=t,r=r,vx=vx,vy=vy,vxy=vxy,w=w,Ti=Ti):
...
# ********   Dynamic circular      ***********
    ...
    def fcentripeta(self,kangle,keval=True,kope=''):
        fx=self.x_res()*cos(kangle)
        fy=self.y_res()*sin(kangle)
        fi=self.Ti
        kres=unisymbols(fi+fx-fy)
        if keval:
            kres=self.revalue(kres)
        kres=opemat(kres,kope=kope)
        return(kres)
